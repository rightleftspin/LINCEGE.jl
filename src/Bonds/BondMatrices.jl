struct BondMatrix{W<:AbstractVector,M<:AbstractMatrix{<:Integer}} <: AbstractAdjacencyMatrix
        weight_info::W
        adj_matrix::M
end

function BondMatrix(real_space_lattice::RealSpaceLattice, tiling::Tiling)
        ...
        BondMatrix(neighbors, adj_matrix)
end
...
"""
function weighted_iso_hash(g::BondMatrix)
...
returns the permutation to rearrange the cluster used by Nauty
"""
function unweighted_iso_hash(g::BondMatrix)
...
function isomorphic_hash(
        ...
        g = BondMatrix(Bool[], new_adj_matrix)
