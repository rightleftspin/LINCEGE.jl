...
function simple_nlce(basis, primitive_vectors, order)
    ...
    for nlce_order in 1:order
      ## Generating the lattice itself
      lattice, center = generate_lattice(basis, primitive_vectors, 1, nlce_order + 3)
      ...
      for (key, (cluster, mult)) in isomorphic_counts
          subgraph_dict = counting(enumerate_subclusters(cluster, 1, Vector(1:nv(cluster))), isomorphic_tagging)
          for subgraph_order in 2:(nv(cluster) - 1)
              merge!(subgraph_dict, counting(enumerate_subclusters(cluster, subgraph_order, Vector(1:nv(cluster))), isomorphic_tagging))
          end
