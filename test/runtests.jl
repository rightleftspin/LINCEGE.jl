...
@testset verbose=true "NLCE" begin
    ...
    square_lattice = Dict( "Basis" => [[0, 0]],
                         "Primitive Vectors" => [[1, 0], [0, 1]],
                        )
    
    triangular_lattice = Dict( "Basis" => [[0, 0]],
                         "Primitive Vectors" => [[0.5, sqrt(3)/2], [1, 0]],
                        )
    ...
    @testset "NLCELattice" begin
        ...
        square_nlce_lattice = NLCE.NLCELattice(square_lattice["Basis"], square_lattice["Primitive Vectors"], neighborhood, max_order)
        triangular_nlce_lattice = NLCE.NLCELattice(triangular_lattice["Basis"], triangular_lattice["Primitive Vectors"], neighborhood, max_order)
        ...
        @test length(NLCE.neighbors(triangular_nlce_lattice, [fld(number_verts, 2), 1])) == 2

        @test sum(length, NLCE.neighbors(square_nlce_lattice, [fld(number_verts, 2), 1])) == 6
        @test sum(length, NLCE.neighbors(triangular_nlce_lattice, [fld(number_verts, 2), 1])) == 8
